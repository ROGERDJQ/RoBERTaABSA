...
def get_dataset(dataset_name):
    ...
    if dataset_name.startswith('/'):
        fns = os.listdir(dataset_name)
        train_fp = os.path.join(dataset_name, [fn for fn in fns if 'Train' in fn][0])
        test_fp = os.path.join(dataset_name, [fn for fn in fns if 'Test' in fn][0])
    else:
        ...
        ds_train = {'Restaurants': rest_train,
                    'Laptop': laptop_train, 'Tweets': twitter_train}
        ds_test = {'Restaurants': rest_test,
                   'Laptop': laptop_test, 'Tweets': twitter_test}
        train_fp = ds_train[dataset_name]
        test_fp = ds_test[dataset_name]

    train = list(read_sentence_depparsed(train_fp))
    logger.info('# Read %s Train set: %d', dataset_name, len(train))

    test = list(read_sentence_depparsed(test_fp))
    logger.info("# Read %s Test set: %d", dataset_name, len(test))
    return train, test


def reshapre_dependency_tree_new2(as_start, as_end, dependencies, multi_hop=False, add_non_connect=False, tokens=None, max_hop = 5):
    # 修改计算方式
        ...
        for dep in dependencies:
            if i == dep[1] - 1:  # 如果依赖到了aspect中的词
                ...
                if (dep[2] - 1 < as_start or dep[2] - 1 >= as_end) and dep[2] != 0 and dep[2] - 1 not in dep_idx:
                    ...
                    dep_tag.append('ncon_0')
                    dep_dir.append(1)
                    # else:
                    #     dep_tag.append('<pad>')
                    #     dep_dir.append(0)
                    dep_idx.append(dep[2] - 1)
            elif i == dep[2] - 1:  # 如果是这个词自己
                # not root, not aspect
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    for e in input_data:
        ...
        if 'tokens' in e:
            e['tokens'] = [x for x in e['tokens']]
        else:
            e['tokens'] = [x for x in e['token']]
        aspects = []
        sentiments = []
        froms = []
        tos = []
        ...
        if 'aspect_sentiment' in e:
            for i in range(len(e['aspect_sentiment'])):
                ...
                dep_tag, dep_idx, dep_dir = reshapre_dependency_tree_new2(frm, to, e['dependencies'],
            ...
            for i in range(len(e['aspects'])):
                # old
                # aspect = e['aspect_sentiment'][i][0].lower()
                aspect = ' '.join(e['aspects'][i]['term'])
                # We would tokenize the aspect while at it.
                aspect = word_tokenize(aspect)
                sentiment = sentiments_lookup[e['aspects'][i]['polarity']]
                frm = e['aspects'][i]['from']
                to = e['aspects'][i]['to']
                ...
                total_counter[e['aspects'][i]['polarity']] += 1

                # Unrolling
                all_unrolled.append(
                    {'sentence': e['tokens'], 'tags': pos_class, 'pos_class': pos_class, 'aspect': aspect, 'sentiment': sentiment,
                     'predicted_dependencies': e['head'], 'predicted_heads': e['head'],
                     'from': frm, 'to': to, 'dep_tag': dep_tag, 'dep_idx': dep_idx, 'dep_dir':dep_dir,'dependencies': e['dependencies']})
            # All sentences with multiple aspects and sentiments rolled.
            all_rolled.append(
                {'sentence': e['tokens'], 'tags': e['pos'], 'pos_class': pos_class, 'aspects': aspects, 'sentiments': sentiments,
                 'from': froms, 'to': tos, 'dep_tags': dep_tags, 'dep_index': dep_index, 'dependencies': e['dependencies']})
...
def load_and_cache_vocabs(data, args):
    ...
    if args.embedding_type == 'glove':
        cached_word_vocab_file = os.path.join(
            args.output_dir, 'cached_{}_{}_word_vocab.pkl'.format(os.path.basename(args.dataset_name), args.embedding_type))
        if os.path.exists(cached_word_vocab_file) and args.refresh<2:
            ...
            logger.info('Creating word vocab from dataset %s',
                        os.path.basename(args.dataset_name))
            word_vocab = build_text_vocab(data, lower=args.lower)
        ...
        cached_word_vecs_file = os.path.join(args.output_dir, 'cached_{}_{}_word_vecs.pkl'.format(
            os.path.basename(args.dataset_name), args.embedding_type))
        if os.path.exists(cached_word_vecs_file) and args.refresh<2:
            ...
            word_vecs = load_glove_embedding(
                word_vocab['itos'], args.glove_dir, args.dataset_name, args.embedding_dim)
    ...
    logger.info("In total {} words".format(len(word_vocab['itos'])))
    # Build vocab of dependency tags
    cached_dep_tag_vocab_file = os.path.join(
        pkls_path, 'cached_{}_dep_tag_vocab.pkl'.format(os.path.basename(args.dataset_name)))
    ...
    cached_pos_tag_vocab_file = os.path.join(
        pkls_path, 'cached_{}_pos_tag_vocab.pkl'.format(os.path.basename(args.dataset_name)))
...
import math
def load_glove_embedding(word_list, glove_dir, data_dir, dimension_size):
    if os.path.basename(data_dir) == 'fr':
        fp = os.path.join(glove_dir, 'word2vec_french.txt')
    elif os.path.basename(data_dir) == 'dutch':
        fp = os.path.join(glove_dir, 'word2vec_dutch.txt')
    elif os.path.basename(data_dir) == 'sp':
        fp = os.path.join(glove_dir, 'word2vec_spanish.txt')
    else:
        fp = os.path.join(glove_dir, 'glove.840B.300d.txt')
    glove_words = read_vectors(fp)
    word2offset = {w: i for i, w in enumerate(glove_words)}
    lowered_word2offset = {w.lower(): i for i, w in enumerate(glove_words)}
    word_vectors = []
    found = 0
    for word in word_list:
        if word in word2offset:
            line = linecache.getline(fp, word2offset[word]+1)
            assert(word == line[:line.find(' ')].strip())
            word_vectors.append(np.fromstring(
                line[line.find(' '):].strip(), sep=' ', dtype=np.float32))
            found += 1
        elif word.lower() in lowered_word2offset:
            line = linecache.getline(fp, lowered_word2offset[word.lower()] + 1)
            assert (word.lower() == line[:line.find(' ')].strip().lower())
            word_vectors.append(np.fromstring(
                line[line.find(' '):].strip(), sep=' ', dtype=np.float32))
            found += 1
        elif word == '[PAD]':
            ...
            word_vectors.append(np.zeros(dimension_size, dtype=np.float32))
            # word_vectors.append(
            #     np.random.uniform(-math.sqrt(1/dimension_size), math.sqrt(1/dimension_size), dimension_size))
    logger.info("Get {}/{} pretrained embeds".format(found, len(word_vectors)))
    return word_vectors
...
def build_text_vocab(data, vocab_size=100000, min_freq=0, lower=False):
    counter = Counter()
    sentences = {}
    for d in data:
        if ' '.join(d['sentence']) in sentences:
            continue
        if lower:
            s = [w.lower() for w in d['sentence']]
        else:
            s = d['sentence']
        sentences[' '.join(d['sentence'])] = 1
        counter.update(s)
...
def build_dep_tag_vocab(data, vocab_size=1000, min_freq=0):
    ...
    stoi = defaultdict(_default_unk_index)
    stoi.update({tok: i for i, tok in enumerate(itos)})
    # logger.info("Dependency label vocab:", itos)
...
class ASBA_Depparsed_Dataset(Dataset):
    ...
    def convert_features_roberta(self, i):
        """
        BERT features.
        convert sentence to feature.
        """
        ...
        for word in self.data[i]['sentence']:
            word_tokens = self.args.tokenizer.tokenize(word, add_prefix_space=True)
            token_idx = len(tokens)
            tokens.extend(word_tokens)
            # word_indexer is for indexing after bert, feature back to the length of original length.
            word_indexer.append(token_idx)
        ...
        # aspect
        for word in self.data[i]['aspect']:
            word_aspect_tokens = self.args.tokenizer.tokenize(word, add_prefix_space=True)
            token_idx = len(aspect_tokens)
            aspect_tokens.extend(word_aspect_tokens)
            aspect_indexer.append(token_idx)
        ...
        input_cat_ids = input_ids + input_aspect_ids[1:]
        segment_ids = [0] * len(input_ids) + [0] * len(input_aspect_ids[1:])
    ...
    def convert_features(self):
        ...
        for i in range(len(self.data)):
            if self.args.embedding_type == 'glove':
                self.data[i]['sentence_ids'] = [self.word_vocab['stoi'][w.lower() if self.args.lower else w]
                                                for w in self.data[i]['sentence']]
                self.data[i]['aspect_ids'] = [self.word_vocab['stoi'][w.lower() if self.args.lower else w]
                                              for w in self.data[i]['aspect']]
            elif self.args.embedding_type == 'elmo':
                self.data[i]['sentence_ids'] = self.data[i]['sentence']
                self.data[i]['aspect_ids'] = self.data[i]['aspect']
            elif self.args.embedding_type == 'roberta':
